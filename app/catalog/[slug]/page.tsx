        ...
        <div className="shell">
          ...
          <div className="mt-8 grid gap-10 lg:grid-cols-[1.15fr_.85fr]">
            ...
            <div>
              ...
              <dl className="mt-8 grid grid-cols-2 border-l border-t border-black/15">
                {item.specs.map(([label, value]) => <div key={label} className="border-b border-r border-black/15 p-4"><dt className="text-sm text-muted-foreground">{label}</dt><dd className="mt-1 text-xl font-black">{value}</dd></div>)}
              </dl>
              <div className="mt-7 border-l-4 border-brand bg-[#f3f4f3] p-6"><p className="text-sm text-muted-foreground">Стоимость смены</p><p className="mt-1 text-4xl font-black">от {item.price.toLocaleString("ru-RU")} ₽</p><p className="mt-2 text-sm text-muted-foreground">{item.minimum}. Итоговая цена зависит от адреса и условий работы.</p></div>
